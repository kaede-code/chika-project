...
<div class="content-box" style="margin-top:14px; max-width:700px;">
    <form method="POST" action="{{ route('admin.products.update', $product->id) }}" enctype="multipart/form-data" style="display:flex; flex-direction:column; gap:12px;">
        @csrf
        @method('PUT')
        ...
        <div class="field">
            ...
            <div class="upload-area" id="uploadArea">
                ...
                <div id="uploadPlaceholder" style="display:{{ $product->gambar ? 'none' : 'block' }};">
                ...
                <div id="uploadPreview" style="display:{{ $product->gambar ? 'block' : 'none' }}; position:relative;">
                    <img id="imagePreview" src="{{ $product->gambar ? asset('storage/'.$product->gambar) : '' }}" alt="Preview" style="width:100%; max-height:200px; object-fit:contain; border-radius:8px;" />
        ...
        <div class="field">
            ...
            <input id="harga" name="harga" type="number" min="0.01" step="0.01" value="{{ old('harga', $product->harga) }}" required>
        ...
        <div class="field">
            ...
            <select id="kategori" name="kategori" required>
                <option value="Jus" {{ old('kategori', $product->kategori) === 'Jus' ? 'selected' : '' }}>Jus</option>
                <option value="Salad" {{ old('kategori', $product->kategori) === 'Salad' ? 'selected' : '' }}>Salad</option>
            </select>
        ...
        <div style="display:flex; gap:10px; margin-top:6px;">
            ...
            <button type="submit" class="btn btn-primary" style="flex:1; text-align:center;">Simpan Perubahan</button>
        </div>
    </form>

    <form method="POST" action="{{ route('admin.products.destroy', $product->id) }}" onsubmit="return confirm('Yakin ingin menghapus produk ini?')" style="margin-top:10px;">
        @csrf
        @method('DELETE')
        <button type="submit" class="btn btn-danger" style="width:100%;">Hapus Produk</button>
    </form>
</div>
